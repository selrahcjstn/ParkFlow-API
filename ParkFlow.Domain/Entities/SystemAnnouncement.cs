...
namespace ParkFlow.Domain.Entities
{
    public class SystemAnnouncement : BaseEntity
    {
        ...
        public SystemAnnouncement(Guid createdBy, string message, string iconType = "caution", bool isActive = true)
        {
            CreatedBy = createdBy;
            Message = message;
        ...
        public void Update(string message, string iconType, bool isActive)
        {
            Message = message;
            IconType = string.IsNullOrWhiteSpace(iconType) ? "caution" : iconType.ToLower().Trim();
            IsActive = isActive;
